/*
 ...
 Name: 040-validate-edm-metadata-preload.groovy
 Owner:
 Created: 2026-09-08
 Updated: 2026-09-08
 Purpose:
   - Validate exported EDM dimension files before import to an EPM application
   - Block load when EDM files diverge from target EPM metadata retrieved via REST
   - Exclude shared members from the EPM baseline (shared hierarchies are not synced from EDM)
 Inputs:
   ...
   - Optional RTPs: bypassDeltaCheck, bypassMemberMatchCheck
 Outputs:
   - Pass/fail gate for metadata load pipeline
*/

import groovy.json.JsonSlurper

class ValidationConfig {
    static final boolean FAIL_ON_MISSING_REFERENCE_DIMENSION = true
    static final int MAX_ABSOLUTE_DELTA = 250
    static final BigDecimal MAX_PERCENT_DELTA = 0.20G
    static final int MAX_MISMATCH_SAMPLE = 15
    // Configure these as EPM runtime prompts (replacement variables) on the rule.
    // true bypasses the corresponding validation gate.
    static final String RTP_BYPASS_DELTA_CHECK = "{bypassDeltaCheck}"
    static final String RTP_BYPASS_MEMBER_MATCH_CHECK = "{bypassMemberMatchCheck}"
    static final boolean DEFAULT_BYPASS_DELTA_CHECK = false
    static final boolean DEFAULT_BYPASS_MEMBER_MATCH_CHECK = false
    ...
    // Dimension list to validate — replace with the target application's dimensions.
    static final List<String> DIMENSIONS = [
        "Account",
        "Entity"
    ]
    // Optional source-specific suffix overrides when EDM and target EPM naming differs.
    // Example: EDM export uses "Accounts" but EPM REST uses "Account".
...
class BaselineDeltaEvaluator {
    Map<String, Object> evaluate(String dimensionName, int sourceCount, Integer baselineCount) {
        if (baselineCount == null) {
            return [
                status  : "missing-baseline",
                message : "No baseline count found for ${dimensionName}",
                deltaAbs: 0,
                deltaPct: 0G
            ]
        }

        int deltaAbs = Math.abs(sourceCount - baselineCount.intValue())
        BigDecimal deltaPct = baselineCount == 0
            ? (sourceCount == 0 ? 0G : 1G)
            : (new BigDecimal(deltaAbs) / new BigDecimal(Math.abs(baselineCount)))

        boolean blocked = (deltaAbs > ValidationConfig.MAX_ABSOLUTE_DELTA) || (deltaPct > ValidationConfig.MAX_PERCENT_DELTA)
        return [
...
Set<String> extractDistinctMembersFromEpmDimensionApi(String applicationName, String dimensionName) {
    ...
    println("EPM dimension ${dimensionName}: excluded ${skippedShared.size()} shared members from baseline (shared hierarchies are not synced)")
...
boolean parseBooleanToken(String rawValue, boolean defaultValue, String settingLabel) {
    if (rawValue == null || rawValue.trim().isEmpty()) {
        return defaultValue
    }
    String token = rawValue.trim().toLowerCase()
    if (["true", "t", "yes", "y", "1"].contains(token)) {
        return true
    }
    if (["false", "f", "no", "n", "0"].contains(token)) {
        return false
    }
    throwVetoException(
        "Invalid boolean value '${rawValue}' for ${settingLabel}. " +
        "Use one of: true/false, yes/no, y/n, 1/0."
    )
    return defaultValue
}

String readRuntimePromptValue(String rtpName) {
    try {
        if (rtps == null) {
            return null
        }
        Object rtpEntry = rtps.get(rtpName)
        if (rtpEntry == null) {
            return null
        }
        try {
            Object enteredValue = rtpEntry.getClass().getMethod("getEnteredValue").invoke(rtpEntry)
            return enteredValue == null ? null : enteredValue.toString().trim()
        } catch (Exception ignored) {
            return rtpEntry.toString().trim()
        }
    } catch (Exception ignored) {
        return null
    }
...
boolean resolveBypassFlag(String rtpName, boolean defaultValue, String label) {
    String rawValue = readRuntimePromptValue(rtpName)
    boolean resolved = parseBooleanToken(rawValue, defaultValue, label)
    String source = (rawValue == null || rawValue.isEmpty()) ? "default" : "rtp:${rtpName}"
    println("Validation toggle ${label}=${resolved} (source=${source})")
    return resolved
}
...
void runPreloadValidation() {
    BaselineDeltaEvaluator evaluator = new BaselineDeltaEvaluator()
    List<String> repositoryFiles = listRepositoryFileNames()
    boolean bypassDeltaCheck = resolveBypassFlag(
        ValidationConfig.RTP_BYPASS_DELTA_CHECK,
        ValidationConfig.DEFAULT_BYPASS_DELTA_CHECK,
        "bypassDeltaCheck"
    )
    boolean bypassMemberMatchCheck = resolveBypassFlag(
        ValidationConfig.RTP_BYPASS_MEMBER_MATCH_CHECK,
        ValidationConfig.DEFAULT_BYPASS_MEMBER_MATCH_CHECK,
    ...
    ValidationConfig.DIMENSIONS.each { dimName ->
        ...
        String edmFilePath
        try {
            edmFilePath = resolveDimensionFileBySuffix(
                repositoryFiles,
            ...
            throwVetoException(
                "EDM extract file not found for ${dimName} under ${ValidationConfig.EDM_EXTRACT_ROOT}. " +
                "Expected suffix *${dimName}{${ValidationConfig.EDM_ALLOWED_EXTENSIONS.join(',')}}. " +
        ...
        Set<String> baselineMembers = null
        try {
            baselineMembers = extractDistinctMembersFromEpmDimensionApi(operation.application.name, epmDimensionName)
        } catch (Exception ex) {
            if (ValidationConfig.FAIL_ON_MISSING_REFERENCE_DIMENSION) {
                blockedMessages.add("[${dimName}] EPM dimension missing/unreadable via REST (${epmDimensionName}). ${ex.message}".toString())
                return
            }
        }
        Integer baselineCount = baselineMembers == null ? null : baselineMembers.size()
        Map result = evaluator.evaluate(dimName, sourceCount, baselineCount)

        if ("missing-baseline".equals(result.status)) {
            String msg = "[${dimName}] baseline dimension missing in target EPM metadata; source count=${sourceCount}"
            if (ValidationConfig.FAIL_ON_MISSING_REFERENCE_DIMENSION) {
                blockedMessages.add(msg.toString())
            } else {
                passMessages.add("${msg}; continuing due to FAIL_ON_MISSING_REFERENCE_DIMENSION=false".toString())
            }
            return
        }

        Map comparison = compareMemberSets(sourceMembers, baselineMembers)
        ...
        if (!targetContainedInSource) {
            String mismatchMessage = (
                ...
                "edmCount=${sourceCount}, epmCount=${baselineCount}"
            ).toString()
            if (bypassMemberMatchCheck) {
                passMessages.add("[${dimName}] MEMBER MATCH CHECK BYPASSED. ${mismatchMessage}".toString())
            } else {
        ...
        String baseMsg = "[${dimName}] edmFile=${edmFilePath}, epmDimension=${epmDimensionName}, baseline=${result.baselineCount}, source=${result.sourceCount}, deltaAbs=${result.deltaAbs}, deltaPct=${deltaPct.setScale(2, BigDecimal.ROUND_HALF_UP)}%, edmOnly=${previewMemberSet(edmOnlyMembers)}"
        boolean deltaBlocked = "blocked".equals(result.status)
        if (deltaBlocked && !bypassDeltaCheck) {
            blockedMessages.add(baseMsg.toString())
        } else if (deltaBlocked && bypassDeltaCheck) {
            passMessages.add("[${dimName}] DELTA CHECK BYPASSED. ${baseMsg}".toString())
        } else {
            passMessages.add(baseMsg.toString())
        }
    }

    passMessages.each { println("PRECHECK OK: ${it}") }
    if (!blockedMessages.isEmpty()) {
        blockedMessages.each { println("PRECHECK BLOCK: ${it}") }
        throwVetoException(
            "Metadata preload validation failed. One or more dimensions exceeded thresholds (MAX_ABSOLUTE_DELTA=${ValidationConfig.MAX_ABSOLUTE_DELTA}, MAX_PERCENT_DELTA=${ValidationConfig.MAX_PERCENT_DELTA})."
        )
    }
    ...
    println("Metadata preload validation passed for all configured dimensions.")
}

runPreloadValidation()
